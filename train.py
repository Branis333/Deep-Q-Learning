...
# Placeholder grid for later tuning
...
def print_hyperparam_table():
    ...
    header = [
        "learning_rate", "gamma", "batch_size", "buffer_size", "train_freq",
        "target_update_interval", "exploration_initial_eps",
        "exploration_final_eps", "exploration_fraction",
    ]
...
def hyperparameter_sweep(
    ...
    tune_csv: str = "tune_results.csv",
) -> None:
    """Run a full combinatorial sweep and write a CSV table."""
    lr_vals      = [1e-4, 5e-5, 2e-4]
    gamma_vals   = [0.99, 0.98, 0.995]
    batch_vals   = [32, 64, 128]
    eps_configs  = [
        (1.0, 0.01, 0.10),  # default
        (1.0, 0.05, 0.20),  # slower decay, higher final eps
        (1.0, 0.01, 0.05),  # faster decay
    ]

    header = [
        "policy", "lr", "gamma", "batch", "eps_start", "eps_end", "eps_frac",
        "train_time_min", "eval_mean_reward", "eval_std_reward", "seed"
    ]
    results_path = Path(tune_csv)
    ...
    trial = 0
    for lr, gamma, batch, (eps_s, eps_e, eps_f) in itertools.product(
        lr_vals, gamma_vals, batch_vals, eps_configs
    ):
        trial += 1
        print(f"\n=== TRIAL {trial:03d} | "
              f"lr={lr:.0e} γ={gamma} batch={batch} "
              f"ε[{eps_s}→{eps_e} over {eps_f*100:.0f}%] ===")

        hp = DQNHyperParams(
            learning_rate=lr,
            gamma=gamma,
            batch_size=batch,
            exploration_initial_eps=eps_s,
            exploration_final_eps=eps_e,
        ...
        row = [
            ...
            lr, gamma, batch,
            eps_s, eps_e, eps_f,
            metrics.get("train_time_min", np.nan),
            metrics.get("eval_mean_reward", np.nan),
            metrics.get("eval_std_reward", np.nan),
            seed + trial,
        ]
        with open(results_path, "a", newline="") as f:
            csv.writer(f).writerow(row)

    print(f"\nTuning finished → results saved to **{results_path.resolve()}**")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--tune", action="store_true", help="Run full hyperparameter sweep (uses --policy).")
...
def main():
    ...
    if args.tune:
        print("[MODE] Hyper-parameter sweep")
